...
def ejecutar_programa(params, nombre="cga_izhi_onemax"):
...
def guardar_mejor(params, fitness, media=None):
    ...
    with open("mejores_parametros_random.txt", "w") as f:
    ...
    with open("mejores_parametros_random.json", "w") as f:
        json.dump({
            "fitness_medio": media if media else fitness,
...
if __name__ == "__main__":
    ...
    if not os.path.exists("./cga_izhi_onemax"):
        print("❌ ERROR: No se encuentra el ejecutable './cga_izhi_onemax'")
        print("Compila el programa primero con: gcc cga_izhi_onemax.c -o cga_izhi_onemax -lm")
        exit(1)
    ...
    if os.path.exists("mejores_parametros_random.json"):
        try:
            with open("mejores_parametros_random.json") as f:
                data = json.load(f)
